...
def update_dependencies():
    ...
    if par['architecture'] == 'LSTM':
        ...
        par['Wf_r_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_latent'], par['n_hidden']]))
        par['Wi_r_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_latent'], par['n_hidden']]))
        par['Wo_r_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_latent'], par['n_hidden']]))
        par['Wc_r_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_latent'], par['n_hidden']]))
    ...
    # latent variables
    par['W_mu_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_hidden'], par['n_latent']]))
    par['W_sigma_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_hidden'], par['n_latent']]))
    par['W_r_out_init'] =  np.float32(np.random.uniform(-c, c, size = [par['n_hidden'], par['n_input']]))
    par['b_mu_init'] = np.zeros((1, par['n_latent']), dtype = np.float32)
    par['b_sigma_init'] = np.zeros((1, par['n_latent']), dtype = np.float32)
    par['b_r_out_init'] = np.zeros((1, par['n_input']), dtype = np.float32)
